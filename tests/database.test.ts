...
describe('Currency Conversion Utils', () => {
  ...
  test('convertAmount handles same currency conversion', () => {
    const result = convertAmount(500, 'YER', 'YER', DEFAULT_RATES);
    assert.equal(result, 500);
  });
});

describe('Database Engine & Relational Logic', () => {
  test('addAccount creates a new account with default currency and status', () => {
  ...
  test('getAccountBalance calculates buyer receivables correctly', () => {
    const db = new Database();
  ...
  test('addDailyLedgerEntry automatically creates relational transaction when linked to an account', () => {
    const db = new Database();
    const acc = db.addAccount({
  ...
  test('deleteAccount soft-deletes account and moves it to deletedAccounts', () => {
    const db = new Database();
    const acc = db.addAccount({
      name: 'حساب سيحذف',
      phone: '+967770003344',
  ...
  test('addInvoice creates invoice record and optional ledger transaction', () => {
    const db = new Database();
    const acc = db.addAccount({
      name: 'عميل الفاتورة',
      phone: '+967770005566',
