...
export class Database {
  ...
  exportState() {
    return {
      ...
      gatewayConfig: this.gatewayConfig || DEFAULT_GATEWAY_CONFIG,
      triggeredMessages: this.triggeredMessages || [],
      primaryCurrency: this.primaryCurrency || 'YER',
      exchangeRates: this.exchangeRates || DEFAULT_RATES,
      lastRatesUpdate: this.lastRatesUpdate || '',
      restrictToAdmin: !!this.restrictToAdmin,
      appAccentColor: this.appAccentColor || 'slate',
      appBorderShape: this.appBorderShape || 'rounded-2xl',
      appBrandIcon: this.appBrandIcon || 'Building2',
      appBackgroundImage: this.appBackgroundImage || '',
      appBackgroundOpacity: this.appBackgroundOpacity ?? 5,
      printCompanyName: this.printCompanyName || 'نظام أنس المحاسبي المطور',
      printPhone: this.printPhone || '+967 774 928 318',
      printAddress: this.printAddress || 'اليمن - صنعاء - شارع الستين الرئيسي',
      printTaxNumber: this.printTaxNumber || '99827-Tax-YE',
      printHeaderNote: this.printHeaderNote || 'كشف حساب مالي تفصيلي معتمد',
      printFooterNote: this.printFooterNote || 'شاكرين ثقتكم الثمينة بنا. تم استخراج هذا السند إلكترونياً من نظام ANAS الذكي لضمان الدقة والأمان.',
      printThemeColor: this.printThemeColor || 'indigo',
      printShowBalance: this.printShowBalance ?? true,
      printShowSignature: this.printShowSignature ?? true,
      printShowWatermark: this.printShowWatermark ?? true,
      printPaperSize: this.printPaperSize || 'A4',
      showWidgetTotalSales: this.showWidgetTotalSales ?? true,
      showWidgetActiveAccounts: this.showWidgetActiveAccounts ?? true,
      showWidgetAlerts: this.showWidgetAlerts ?? true,
